...
class TestSolve(TestCase):
    ...
    def test_get_horizontal_values(self):
        self.assertEqual(get_horizontal_values(0, 0, self.validation), {1, 2})
        self.assertEqual(get_horizontal_values(1, 1, self.validation), {3})
        self.assertEqual(get_horizontal_values(3, 3, self.validation), {1, 3, 4})

    def test_get_vertical_values(self):
        self.assertEqual(get_vertical_values(0, 0, self.validation), {1})
        self.assertEqual(get_vertical_values(1, 1, self.validation), {1})
        self.assertEqual(get_vertical_values(3, 3, self.validation), {2, 3, 4})
